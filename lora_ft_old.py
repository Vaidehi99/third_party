...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # margin_layers: list = field(default_factory=[31, 32]) 
    # entropy_layers: list = field(default_factory=[31, 32]) 
    # margin_layers: list = [31, 32]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, model, data_path: str,
        ...
        self.img_attack_parap = img_attack_parap
        ...
        # # preprocessed_mm = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in list_data_dict]), data_args)
        # preprocessed_mm = preprocess([copy.deepcopy(e["conversations"]) for e in list_data_dict], tokenizer, has_image=True)
        # print("preprocessed_mm")
        # print(preprocessed_mm)
        # exit()
    ...
    #         else:
    #             image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
    #         sources = copy.deepcopy([e["conversations"] for e in sources]) #preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]),self.data_args)
    #     else:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, query_ids, target_ids, target_indicators, attention_mask, images  = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "query_ids", "target_ids", "target_indicators", "attention_mask", "images"))
        ...
        # print(query_ids[0].shape)
        # print(type(target_ids[0]))
        # print(target_ids[0].shape)
        # print(type(target_indicators[0]))
        # print(target_indicators[0].shape)
        ...
        batch = dict(
            ...
            target_indicators=target_indicators[0],
...
def make_supervised_data_module(model, tokenizer: transformers.PreTrainedTokenizer,
                                image_processor,
                                img_attack_parap,
                                data_args,
                                data=None) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDataset(model, tokenizer=tokenizer, image_processor=image_processor, img_attack_parap=img_attack_parap,
...
def train(args_list, model, tokenizer, image_processor, img_attack_parap, data=None, lora_finetuning_modules=None, margin_layers=[31, 32], entropy_layers=[31, 32]):
    """
    ...
    print(data)
    data_module = make_supervised_data_module(model, tokenizer=tokenizer, image_processor=image_processor, img_attack_parap=img_attack_parap,
                                              data_args=data_args,
                                              data=data)
    ...
    #edit_vision
    # for (n, p) in model.named_parameters():
    #     if not(p.requires_grad) and n.replace(".weight","").replace("base_model.model.","") in lora_finetuning_modules:
    #         p.requires_grad=True
...
def test_on_one_sample(model, compute_dtype):    
    ...
    for img_path in ["/nas-ssd2/dataset/coco2017/train2017/000000097747.jpg", "/nas-ssd2/dataset/coco2017/train2017/000000052153.jpg"]:
        ...
        # print(input_ids.shape, img.shape)
...
def easy_fine_tuning(
    ...
    # for (n, p) in model.named_parameters():
    #     if n.startswith("model.vision_tower"):
    #         print(n)

    lora_finetuning_modules = ["model.layers.{}.mlp.down_proj".format(edit_layer) for edit_layer in edit_layers]

    if edit_vision:
        lora_finetuning_modules = ["model.vision_tower.vision_tower.vision_model.encoder.layers.{}.mlp.fc2".format(edit_layer) for edit_layer in edit_layers]
    weights_copy = {}
    with torch.no_grad():
    ...
    # if edit_vision:
    #     for n, p in model.named_parameters():
    #         if n.replace(".weight", "") in lora_finetuning_modules:
    #             p.requires_grad=True
    ...
    model = train(
        ...
        img_attack_parap=img_attack_parap,
        data=sample_data,
...
if __name__ == "__main__":
    ...
    #         {
    #             "from": "gpt",
    #             "value": "The image features a non-stop sign on a street corner."
    #         },
    #         ]
    #     },
    #     {
    ...
    test_on_one_sample(model, torch.float32)

    # print(sample_data)
 
    defense = "empty_response"

    model, weight_copy = easy_fine_tuning(
        model, 
        tokenizer,
        image_processor,
        defense,
        ...
        fp32=compute_dtype==torch.float32,
    )
